...
function TooltipProvider({
  ...
  const showTooltip = React.useCallback(
    (data: TooltipData) => {
      console.log("TooltipProvider: showTooltip", data.id);
      if (timeoutRef.current) clearTimeout(timeoutRef.current);
      if (currentTooltip !== null) {
        console.log("TooltipProvider: showTooltip: immediate show");
        setCurrentTooltip(data);
        return;
      }
      const now = Date.now();
      const delay = now - lastCloseTimeRef.current < closeDelay ? 0 : openDelay;
      console.log("TooltipProvider: showTooltip: scheduling with delay", delay);
      timeoutRef.current = window.setTimeout(
        () => {
          console.log("TooltipProvider: showTooltip: timeout fired, setting state");
          setCurrentTooltip(data);
        },
        delay,
      );
...
function TooltipOverlay() {
  ...
  console.log("TooltipOverlay: rendering", { data: rendered.data, ready, open: rendered.open, x, y });
...
function TooltipTrigger({
  ...
  const handleOpen = React.useCallback(() => {
    console.log("TooltipTrigger: handleOpen", id, triggerRef.current);
  ...
  const handleMouseEnter = React.useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => {
      console.log("TooltipTrigger: mouseenter", id);
      onMouseEnter?.(e);
      handleOpen();
    },
    [handleOpen, onMouseEnter, id],
  );
