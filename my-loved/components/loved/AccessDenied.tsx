...
export default function AccessDenied({ gradient }: AccessDeniedProps) {
  return (
    <div className={`min-h-screen flex flex-col items-center justify-center bg-gradient-to-br ${gradient} p-6 relative overflow-hidden text-center`}>
      ...
      <div className="relative max-w-md p-8 rounded-3xl bg-white/40 dark:bg-black/35 backdrop-blur-xl border border-white/20 shadow-2xl flex flex-col items-center gap-6 animate-scale-up">
        {/* Glowing lock badge */}
        ...
        <p className="text-zinc-600 dark:text-zinc-300 text-sm leading-relaxed font-sans">
          You cannot access our special memories and story details without logging in first. Please sign in to enter our kingdom.
        </p>

        <div className="flex flex-col gap-3.5 w-full mt-2">
          <SignInButton mode="modal">
            <button className="w-full py-3 bg-gradient-to-r from-rose-500 to-pink-500 hover:brightness-105 text-white font-semibold rounded-full shadow-md hover:shadow-lg transition-all cursor-pointer text-sm">
              Sign In to Enter 💖
            </button>
          </SignInButton>
          ...
          <Link
            href="/"
            className="w-full py-3 bg-white/50 dark:bg-zinc-800/30 border border-zinc-200/50 dark:border-zinc-800/50 hover:bg-white/70 dark:hover:bg-zinc-800/50 text-zinc-700 dark:text-zinc-300 font-medium rounded-full text-xs transition-colors cursor-pointer"
          >
            Go Back Home
          </Link>
        </div>
