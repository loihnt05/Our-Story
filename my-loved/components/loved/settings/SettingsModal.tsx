...
import PartnerSettingsModal from "@/components/loved/settings/PartnerSettingsModal";
...
export default function SettingsModal({
  ...
  return (
    <>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
        <div className="w-full max-w-lg rounded-3xl bg-white dark:bg-zinc-900 shadow-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden max-h-[90vh] flex flex-col animate-scale-up">
          ...
          <div className="px-6 py-5 border-b border-zinc-200/50 dark:border-zinc-800/50 flex justify-between items-center">
            ...
            <button
              onClick={onClose}
              className="text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200 transition-colors cursor-pointer border-none bg-transparent"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          <div className="px-6 py-4.5 bg-zinc-50 dark:bg-zinc-950/40 border-t border-zinc-200/50 dark:border-zinc-800/50 flex justify-end">
            <button
              onClick={onClose}
              className="px-6 py-2.5 rounded-full bg-rose-500 hover:bg-rose-600 text-white font-medium shadow-sm transition-colors text-sm cursor-pointer border-none"
            >
      ...
      {/* DEDICATED PARTNER & ME PROFILES POPUP MODAL */}
      {showPartnerModal && (
